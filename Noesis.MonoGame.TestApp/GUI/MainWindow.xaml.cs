...
namespace Noesis.MonoGame.TestApp.GUI
{
    ...
    public partial class MainWindow : Window
    {
        public MainWindow()
        {
            ...
            SizeChanged += (s, e) =>
            {
                ...
                {
                    SampleOffset.Width = SelectorBar.Width;
                    SelectorExpanderButton.Visibility = Visibility.Hidden;
                    SelectorExpanderButton.IsChecked = false;
                    SelectorExpanderButton.IsChecked = true;

                    scale = (float)Math.Max(1.0F, e.NewSize.Width / (1280.0F * dpi)) * dpi;
                }
                else
                {
                    SampleOffset.Width = 0.0F;
                    SelectorExpanderButton.IsChecked = true;
                    SelectorExpanderButton.IsChecked = false;
                    SelectorExpanderButton.Visibility = Visibility.Visible;

                    scale = (float)Math.Max(1.0F, e.NewSize.Height / (720.0F * dpi)) * dpi;
                }

                ScaleTransform rootScale = (ScaleTransform)LayoutRoot.LayoutTransform;
        ...
        {
            Noesis.GUI.LoadComponent(this, "GUI/MainWindow.xaml");

            LayoutRoot = (Panel)FindName("LayoutRoot");

            SamplePanel = (Panel)FindName("SamplePanel");
            SampleContainer1 = (ContentControl)FindName("SampleContainer1");
            SampleContainer2 = (ContentControl)FindName("SampleContainer2");

            SampleOffset = (FrameworkElement)FindName("SampleOffset");
            SelectorBar = (FrameworkElement)FindName("SelectorBar");
            SelectorExpanderButton = (ToggleButton)FindName("SelectorExpanderButton");
        }
        ...
        void UpdateSample()
        {
            string sampleName = string.Format("GUI/Samples/{0}.xaml", _selectedSample.Name);

            FrameworkElement sample = (FrameworkElement)Application.LoadComponent(new Uri(sampleName, UriKind.Relative));

            if (sample != null)
            {
                if (SampleContainer1.Content == null)
                {
                    SampleContainer1.Content = sample;
                }
                else
                {
                    SampleContainer2.Content = sample;
                }
            }
        }
        ...
        void UpdateTheme()
        {
            ...
            ResourceDictionary resources = SamplePanel.Resources;
        ...
        ResourceDictionary GetTheme(string color, string accent)
        {
            ...
            Uri uri = new(System.IO.Path.Combine(Environment.CurrentDirectory, @"GUI\Theme", themeName), UriKind.Absolute);
